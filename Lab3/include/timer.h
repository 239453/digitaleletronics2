...
/** @brief Stop timer, prescaler 000 --> STOP */
#define TIM0_STOP      TCCR0B &= ~((1<<CS02) | (1<<CS01) | (1<<CS0));

/** @brief Set overflow 4ms, prescaler 001 --> 1 */
#define TIM0_OVF_16US   TCCR0B &= ~((1<<CS02) | (1<<CS01)); TCCR1B |= (1<<CS00);

/** @brief Set overflow 33ms, prescaler 010 --> 8 */
#define TIM0_OVF_128US  TCCR0B &= ~((1<<CS02) | (1<<CS00)); TCCR1B |= (1<<CS01);

/** @brief Set overflow 262ms, prescaler 011 --> 64 */
#define TIM0_OVF_1MS TCCR0B &= ~(1<<CS02); TCCR1B |= (1<<CS01) | (1<<CS00);

/** @brief Set overflow 1s, prescaler 100 --> 256 */
#define TIM0_OVF_4MS  TCCR0B &= ~((1<<CS00) | (1<<CS01)); TCCR1B |= (1<<CS02);

/** @brief Set overflow 4s, prescaler // 101 --> 1024 */
#define TIM0_OVF_16MS  TCCR0B &= ~(1<<CS01); TCCR1B |= (1<<CS02) | (1<<CS00);
